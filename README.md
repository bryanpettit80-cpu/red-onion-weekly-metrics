...
In the named Dropbox deployment, the launcher first verifies that `Red Onion Weekly Metrics Automation` is a clean Git checkout on `main`, that `HEAD` matches the checkout's local `origin/main` reference, and that `_program` contains no ignored `.pyc` or `.pyo` files that could bypass Git review. A failed release check stops before the run creates folders, installs packages, builds workbooks, or moves source files. The check is intentionally local and does not fetch from GitHub; a technical maintainer must update and verify the deployed checkout as a separate release step.
